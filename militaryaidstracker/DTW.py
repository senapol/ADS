...
for selected_aid in aid_categories:
    scaler = MinMaxScaler()
    ...
    for i, j in filtered_alignment:
        plt.plot([i, j], [aid_series[i], move_series[j]], color='lightgray', linewidth=0.6)

    plt.plot(aid_series, label=f"{selected_aid} (normalized)", color='orange')
    plt.plot(move_series, label="Frontline Movement (normalized)", color='blue')
