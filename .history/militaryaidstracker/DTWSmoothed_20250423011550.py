...
# zero‐mean your series
a0 = aid_z - aid_z.mean()
f0 = front_z - front_z.mean()
N  = len(a0)

# full cross‐correlation
corr_full = correlate(a0, f0, mode="full")

# build the lags array for mode="full"
lags_full = np.arange(-N+1, N)

# pick only lags between -5 and +5
mask = (lags_full >= -5) & (lags_full <= +5)
...
best_corrval = corr_small[best_idx] / (np.std(a0)*np.std(f0)*N)
