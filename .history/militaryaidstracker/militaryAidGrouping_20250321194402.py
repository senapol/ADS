...
def aggregate_tot_value_eur(group):
    ...
    tot_eur = int(total_usd * exchange_rates["USD"]) if not np.isnan(total_usd) else 0
    ...
    if pd.isna(s_reported):
        ...
        adjusted_value = s_reported - tot_eur
...
print(df.loc['items_value_estimate_EUR'].head(10))
