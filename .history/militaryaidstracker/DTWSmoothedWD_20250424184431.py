...
alignment = dtw.warping_path(aid_series, move_series, window=5)
dist = dtw.distance(    aid_series, move_series, window=5)
...
for cat in categories + ['Total aid']:
    ...
    print("Base case", euc)
    print("Per point: ", euc/len(aid_series))

    # 2. compute DTW distance (not the full path)
    alignment = dtw.warping_path(aid_series, move_series, window=3)
    dist = dtw.distance(aid_series, move_series, window=3)

    print("Lenght of path / len of series: ", len(alignment)/len(aid_series))
