...
for cat in categories:
    # 1. extract & normalize this category vs frontline
    X = merged_df[[cat, "area_sq_km"]].fillna(0).values
    scaled = MinMaxScaler().fit_transform(X)
    aid_s, front_s = scaled[:,0], scaled[:,1]

    # 2. compute DTW distance (not the full path)
    dist = dtw.distance(aid_s, front_s, window=5)
