...
df.loc[mask, "Uncategorised"] = (
    df.loc[mask, "Uncategorised"] - df.loc[mask, "source_reported_value_EUR"] + df.loc[mask, "Humanitarian"]
)
...
target_date = pd.Timestamp('2024-09-02')
# Find the tuple key in the multi-index that matches the target date
target_idx_tuple = [idx for idx in df.index if idx[0] == target_date][0]

# Convert the tuple key to an integer location
target_pos = df.index.get_loc(target_idx_tuple)

# Get the next two rows after the target row
print(next_rows = df.iloc[target_pos: target_pos + 10][cols_to_sum + ['announcement_date']])

grouped = df_original.groupby(pd.Grouper(key='announcement_date', freq='W'))

target_date = pd.Timestamp('2024-09-01')  # adjust this to the exact group key as it appears in the groups
