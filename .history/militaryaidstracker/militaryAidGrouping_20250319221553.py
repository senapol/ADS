...
df = aid_df[['activity_id', 'aid_type_general', 'announcement_date', 'donor', 'item_type', 'item_value_estimate_USD', 'explanation', 
...
dict_weapons = {'.' : None, 'Ammunition for portable defence system' : 'Portable defence system', 'Ammunition for heavy weapon' : 'Heavy weapon',
 ...
 'Heavy Weapon' : 'Heavy weapon', 'ammunition for heavy weapon' : 'Heavy weapon', 'Missile' : 'Heavy weapon', 'Funding, training, services' : 'Military equipment',  'Ammunition' : None} #
...
# print(df[df['item_type'].isna()].count())
# print(df.count())
...
df["remaining_value_EUR"] = df["source_reported_value_EUR"] - df["item_value_estimate_USD"]

# 
df["Financial"] = np.where(
    (df['item_type'] == None) & (df['aid_type_general'] == "Financial"),
