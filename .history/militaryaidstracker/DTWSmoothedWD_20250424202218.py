...
def dtw_asymmetric(a, b, maxlag):
        """
        Compute DTW distance where b[j] may only match a[i] if
        i <= j <= i + maxlag
        """
        n, m = len(a), len(b)
        # DP table with one extra row/col for the base case
        D = np.full((n+1, m+1), np.inf)
        D[0, 0] = 0.0

        for i in range(1, n+1):
            # only allow j from i (no backward) to i+maxlag
            j_start = i
            j_end   = min(m, i + maxlag)
        ...
        while i > 0 or j > 0:
            steps += 1
            # take the predecessor that produced D[i,j]
            prev = min(
                (D[i-1, j]  , (i-1, j)  ),
                (D[i,   j-1], (i,   j-1)),
                (D[i-1, j-1], (i-1, j-1))
        ...
        i, j      = n, m
        path      = [(i-1, j-1)]        # store 0-based indices
        while i > 0 or j > 0:
            # pick the predecessor that produced D[i,j]
            i1, j1 = i-1, j        # insertion
            i2, j2 = i,   j-1      # deletion
            i3, j3 = i-1, j-1      # match
            prev = min(
                (D[i1, j1], (i1, j1)),
                (D[i2, j2], (i2, j2)),
            ...
            i, j = prev
            if i>0 or j>0:                 # skip the virtual (0,0) cell
                path.append((i-1, j-1))    # convert to 0-based
        path.reverse()
...
alignment = dtw.warping_path(aid_series, move_series, window=4)
alignment_dist, alignment_asym = dtw_asymmetric(aid_series, move_series, 7)

plt.figure(figsize=(16,5))
for i, j in alignment_asym:          # <-- use the new path
    plt.plot([i, j], [aid_series[i], move_series[j]],
...
# … rest of your xticks / title / grid …
plt.tight_layout(); plt.show()
dist = dtw.distance(    aid_series, move_series, window=4)

# vertical_costs = [abs(aid_series[i] - move_series[j]) for i,j in alignment]
# print("Sum of vertical costs =", sum(vertical_costs))   # this is dtw.distance
# print("Manually calculated: ", sum(vertical_costs)/len(alignment))

# print("dist/len(alignment) = ", dist/len(alignment))
...
for cat in categories + ['Total aid']:
    ...
    # Example usage:
    maxlag = 7
    dist_aid_leads, path = dtw_asymmetric(aid_series, move_series, maxlag)
    print("Asymmetric DTW distance:", dist_aid_leads)


    euc = np.sum(np.abs(aid_series - move_series))
    print("Base case distance: ", euc)
    print("Base case distance per point: ", euc/len(aid_series))

    # 2. compute DTW distance (not the full path)
    alignment = dtw.warping_path(aid_series, move_series, window=4)
    dist = dtw.distance(aid_series, move_series, window=4)

    print("Lenght of path / len of series (measures if DTW is too flexible): ", len(alignment)/len(aid_series))
