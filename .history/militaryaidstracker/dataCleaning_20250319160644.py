...
aid_df = pd.read_excel(xls, sheet_name="Bilateral Assistance, MAIN DATA", converters={'announcement_date': str})
...
df['announcement_date'] = df['announcement_date'].astype(str).str.replace("until ", "", regex=False).str.replace(r'\s+', '', regex=True)
df["announcement_date_converted"] = pd.to_datetime(df["announcement_date"], errors='coerce')
invalid_dates = df.loc[df['announcement_date_converted'].isna(), 'announcement_date']
print(invalid_dates)
...
print(aggregated_df[['activity_id', 'source_reported_value_EUR', 'item_value_estimate_USD']])
