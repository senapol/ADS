...
for cat in categories + ['Total aid']:
    ...
    b = move_series
    N, M = len(a), len(b)
    maxlag = 7   # you still want ±7 weeks, but only forward

    # build an asymmetric window
    mask = np.zeros((N, M), dtype=bool)
    for i in range(N):
    # allow matches from j=i (no lag) up to j=i+maxlag
        jmin = i
        jmax = min(M, i + maxlag + 1)
        mask[i, jmin:jmax] = True
        print("Category: ", cat)

    dist, paths = dtw.warping_paths(a, b, window=mask)
