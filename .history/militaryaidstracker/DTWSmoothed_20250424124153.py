...
isw_df = pd.read_csv("data/ISW_area_of_control.csv")

isw_df["date"] = pd.to_datetime(isw_df["date"])
frontline_df = isw_df.groupby(pd.Grouper(key='date', freq='W-MON')).agg({
    'area_as_percent_of_Ukraine': 'mean',
    'area_in_square_km': 'mean'
}).reset_index()
frontline_df["area_in_square_km"] = -frontline_df["area_in_square_km"]
...
#  aid_df[categories] = aid_df[categories]/100000000
print(aid_df.count())
...
scaler = MinMaxScaler()
normalized = scaler.fit_transform(merged_df[[selected_aid, "area_in_square_km"]].fillna(0))
...
for cat in categories + ['Total aid']:
    scaler = MinMaxScaler()
    normalized = scaler.fit_transform(merged_df[[cat, "area_in_square_km"]].fillna(0))
    aid_series = normalized[:, 0]
    move_series = normalized[:, 1]
    dates = merged_df["announcement_date"].dt.strftime("%Y-%m-%d")
