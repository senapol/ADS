...
for cat in categories + ['Total aid']:
    ...
    def dtw_asymmetric_with_path(a, b, maxlag):
        n, m = len(a), len(b)
        D = np.full((n+1, m+1), np.inf)
        D[0,0] = 0.
        # populate D as before…

        # now backtrack from (n,m) to (0,0)
        i, j = n, m
        path = [(i,j)]
        while i>0 or j>0:
            # look at neighbors
        ...
        total_cost = D[n,m]
        avg_cost   = total_cost / (len(path)-1)  # minus the (0,0) step
        return total_cost, avg_cost, path

    dist, avg, path = dtw_asymmetric_with_path(aid_series, move_series, maxlag=7)
    print("Sum of L1 costs:", dist)
    print("Mean cost per match:", avg)

    # Example usage:
    maxlag = 7
    dist_aid_leads = dtw_asymmetric_with_path(aid_series, move_series, maxlag)
