...
parser.add_argument('--mode', type=str, default='constant', help='Mode for processing data: constant or percapita')
...
def clean_df(df):
        ...
        df = filter_relevant_countries(df, countries_of_interest['direct'] + countries_of_interest['neighbouring'] + countries_of_interest['baltic'] + countries_of_interest['other'])
        return df

df = process_milex_excel('SIPRI-Milex-data-1948-2023.xlsx')
print(df.columns[df.isna().all()])

df_ukruss = filter_relevant_countries(df, countries_of_interest['direct'])
df_neighbours = filter_relevant_countries(df, countries_of_interest['neighbouring'])
df_baltic = filter_relevant_countries(df, countries_of_interest['baltic'])
df_other = filter_relevant_countries(df, countries_of_interest['other'])
